package com.behtar.lens.internal.analytics
...
internal object FirebaseAnalyticsValidator {
    ...
    fun validateEvent(entry: AnalyticsLogEntry): EventValidationResult {
        ...
        val tooManyParams = if (entry.params.size > MAX_PARAMS_PER_EVENT)
            listOf(Violation.TooManyParams(actual = entry.params.size, max = MAX_PARAMS_PER_EVENT))
        else emptyList()

        val paramViolations = entry.params
    ...
    fun validateUserProperty(entry: UserPropertyEntry): PropertyValidationResult {
        ...
        val propertyViolations = entry.properties
    ...
    private fun validateEventName(name: String): List<Violation> {
        ...
        if (!NAME_REGEX.matches(name))
            violations += Violation.InvalidCharacters(name)
        RESERVED_PREFIXES.firstOrNull { name.startsWith(it) }?.let {
            violations += Violation.ReservedPrefix(it)
        }
        if (name in RESERVED_EVENT_NAMES)
            violations += Violation.ReservedName(name)
        return violations
    }
    ...
    private fun validateParam(key: String, value: Any?): List<Violation> {
        ...
        if (!NAME_REGEX.matches(key))
            violations += Violation.InvalidCharacters(key)
        RESERVED_PREFIXES.firstOrNull { key.startsWith(it) }?.let {
            violations += Violation.ReservedPrefix(it)
        }
        if (key in RESERVED_PARAM_NAMES)
            violations += Violation.ReservedName(key)
    ...
    private fun validateUserPropertyPair(key: String, value: Any?): List<Violation> {
        ...
        if (key in RESERVED_USER_PROPERTY_NAMES)
            violations += Violation.ReservedName(key)
        if (value is String && value.length > MAX_USER_PROP_VALUE_LENGTH)
            violations += Violation.ValueTooLong(actual = value.length, max = MAX_USER_PROP_VALUE_LENGTH)
        return violations
    }
    ...
    private val RESERVED_EVENT_NAMES = setOf(
        ...
        "app_exception", "app_open", "app_remove", "app_store_refund",
        ...
        "notification_receive", "notification_send", "os_update",
        ...
        "begin_checkout", "campaign_details", "earn_virtual_currency",
        "generate_lead", "join_group", "level_end", "level_start", "level_up",
        "login", "post_score", "purchase", "refund", "remove_from_cart",
        "screen_view", "search", "select_content", "select_item", "select_promotion",
        "share", "sign_up", "spend_virtual_currency", "tutorial_begin",
        "tutorial_complete", "unlock_achievement", "view_cart", "view_item",
        "view_item_list", "view_promotion", "view_search_results",
    )

    // https://firebase.google.com/docs/reference/android/com/google/firebase/analytics/FirebaseAnalytics.Param
    private val RESERVED_PARAM_NAMES = setOf(
        "cid", "currency", "customer_id", "customerid", "dclid", "gclid",
        "session_id", "sessionid", "sfmc_id", "sid", "srsltid", "uid",
        "user_id", "userid", "firebase_conversion",
    )
...
internal data class EventValidationResult(
    ...
    val hasViolations: Boolean
        get() = eventNameViolations.isNotEmpty() ||
                tooManyParamsViolation.isNotEmpty() ||
                paramViolations.isNotEmpty()

    companion object {
        val CLEAN = EventValidationResult(
...
internal data class PropertyValidationResult(
    ...
    val hasViolations: Boolean get() = propertyViolations.isNotEmpty()
...
internal sealed class Violation {
    ...
    data class InvalidCharacters(val value: String) : Violation() {
        override fun message() = "\"$value\" contains invalid characters — must match [a-zA-Z][a-zA-Z0-9_]*"
    }
